...
for i in range(1,num_x): #计算本解各个位上的值
	for k in range(0,num_y):
		for j in range (0,num_y):
			l[(i-1)*num_y*num_y+j*num_y+k]=findXX(s[0],(i-1)*num_y*num_y+j*num_y+k)

l2=ex_line(l,num_x,num_y,0,1)
print l2
s[1]=find_num(l2)

#for n in range(pow(2,num_y*num_y*(num_x-1))-1001,pow(2,num_y*num_y*(num_x-1))-1002):
for n in range(514,519):
	
	for i in range(1,num_x): #计算本解各个位上的值
			for k in range(0,num_y):
				for j in range (0,num_y):
					l[(i-1)*num_y*num_y+j*num_y+k]=findXX(n,(i-1)*num_y*num_y+j*num_y+k)
	p=copy.copy(l)
